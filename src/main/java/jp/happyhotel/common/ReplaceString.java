/*
 * @(#)ReplaceString.java 2.00 2004/12/02 Copyright (C) ALMEX Inc. 2004 •¶š—ñ’uŠ·ƒNƒ‰ƒX
 */
...
public class ReplaceString implements Serializable
{
     ...
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @param pattern ’uŠ·‘O‚Ì•¶š—ñ
     * @param replacement ’uŠ·Œã‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    static public String SQLEscape(String input)
    {
        ...
        if ( input != null )
        {
            ...
            if ( input.length() > 0 )
            {
                ...
                if ( input.lastIndexOf( "\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "ƒ\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "„\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "‡\" ) == input.length() - 1 )
                ...
                if ( input.lastIndexOf( "‰\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "Š\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "‹\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "Œ\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "\" ) == input.length() - 1 )
                ...
                if ( input.lastIndexOf( "\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "Ÿ\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "à\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "á\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "â\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "ã\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "ä\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "å\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "æ\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "ç\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "è\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "é\" ) == input.length() - 1 )
                    escapecode = true;
                if ( input.lastIndexOf( "ê\" ) == input.length() - 1 )
                    escapecode = true;
     ...
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     * @see "•ÏŠ·‘ÎÛi&A<A>A&quot;A'A‰üsƒR[ƒhj"
     */
    static public String HTMLEscape(String input)
    {
        if ( input != null )
        {
            // ƒTƒjƒ^ƒCƒY‚³‚ê‚½&‚ğŒ³‚É–ß‚·
            input = replace( input, "&amp;amp;", "&" );
            // ‚·‚Å‚ÉHTMLƒGƒXƒP[ƒv‚³‚ê‚Ä‚¢‚é‚à‚Ì‚ğŒ³‚É–ß‚·(“ñdƒGƒXƒP[ƒv‚ğ–h‚®‚½‚ßj
            input = replace( input, "&amp;", "&" );
            input = replace( input, "&lt;", "<" );
            input = replace( input, "&gt;", ">" );
            input = replace( input, "&quot;", "\"" );
    ...
    /**
     * HTML•¶š—ñ‚Ì’uŠ·‚ğs‚¤ (&¨&amp;) (<¨&lt;) (>¨&gt;) ("¨&quot;) Javascript“à‚É•¶š—ñ‚ğƒZƒbƒg‚·‚éê‡
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     * @see "•ÏŠ·‘ÎÛi&A<A>A&quot;A'A‰üsƒR[ƒhj"
    ...
    /**
     * HTML•¶š—ñ‚Ì’uŠ·‚ğs‚¤ (&¨&amp;) (<¨&lt;) (>¨&gt;) ("¨&quot;)‚Ì‚İ
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     * @see "•ÏŠ·‘ÎÛi&A<A>A&quot;j"
     */
    ...
    /**
     * Œg‘ÑŒü‚¯‚É•¶š—ñ‚Ì’uŠ·‚ğs‚¤ (‘SŠpƒJƒi¨”¼ŠpƒJƒi) (‘SŠp‹L†¨”¼Šp‹L†) (‘SŠp”š¨”¼Šp”š) (‘SŠpƒAƒ‹ƒtƒ@ƒxƒbƒg¨”¼ŠpƒAƒ‹ƒtƒ@ƒxƒbƒg)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    /**
     * ŒŸõƒ[ƒh‚Ìd•¡“o˜^–h~‚Ì‚½‚ß‚É•¶š—ñ‚Ì’uŠ·‚ğs‚¤ (”¼ŠpƒJƒi¨‘SŠpƒJƒi) (‘SŠp”š¨”¼Šp”š) (‘SŠpƒAƒ‹ƒtƒ@ƒxƒbƒg¨”¼ŠpƒAƒ‹ƒtƒ@ƒxƒbƒg) (¬•¶šƒAƒ‹ƒtƒ@ƒxƒbƒg¨‘å•¶šƒAƒ‹ƒtƒ@ƒxƒbƒg)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    static public String replaceKanaHalf(String input)
    {
        ...
        nType = 0;
        // •’Ê‚ÌƒJƒ^ƒJƒi•ÏŠ·—pƒf[ƒ^
        strHankaku = "±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÕÖ×ØÙÚÛÜ¦İ§¨©ª«¬­®¯¤¡°¢£Şß";
        strZenkaku = "ƒAƒCƒEƒGƒIƒJƒLƒNƒPƒRƒTƒVƒXƒZƒ\ƒ^ƒ`ƒcƒeƒgƒiƒjƒkƒlƒm";
        strZenkaku += "ƒnƒqƒtƒwƒzƒ}ƒ~ƒ€ƒƒ‚ƒ„ƒ†ƒˆƒ‰ƒŠƒ‹ƒŒƒƒƒ’ƒ“ƒ@ƒBƒDƒFƒHƒƒƒ…ƒ‡ƒbAB[uv@@";

        // ‘÷‰¹‚ÌƒJƒ^ƒJƒi•ÏŠ·—pƒf[ƒ^
        strHankakuDaku = "¶·¸¹º»¼½¾¿ÀÁÂÃÄÊËÌÍÎ³";
        strZenkakuDaku = "ƒKƒMƒOƒQƒSƒUƒWƒYƒ[ƒ]ƒ_ƒaƒdƒfƒhƒoƒrƒuƒxƒ{ƒ”";

        // ”¼‘÷‰¹‚ÌƒJƒ^ƒJƒi•ÏŠ·—pƒf[ƒ^
        strHankakuHanDaku = "ÊËÌÍÎ";
        strZenkakuHanDaku = "ƒpƒsƒvƒyƒ|";
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ (‘SŠp‹L†¨”¼Šp‹L†) (‘SŠp”š¨”¼Šp”š) (‘SŠpƒAƒ‹ƒtƒ@ƒxƒbƒg¨”¼ŠpƒAƒ‹ƒtƒ@ƒxƒbƒg)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    static public String replaceNumAlphaHalf(String input)
    {
        ...
        strZenMark = "{|–^bIHh”—“•fMijmnCDGFQƒ„OE";
        strHanMark = "+-*/=|!?\"#@$%&'`()[],.;:_<>^¥";
    ...
    static public String replaceKanaFull(String input)
    {
        ...
        // •’Ê‚ÌƒJƒ^ƒJƒi•ÏŠ·—pƒf[ƒ^
        strHankaku = "±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÕÖ×ØÙÚÛÜ¦İ§¨©ª«¬­®¯¤¡°¢£";
        strZenkaku = "ƒAƒCƒEƒGƒIƒJƒLƒNƒPƒRƒTƒVƒXƒZƒ\ƒ^ƒ`ƒcƒeƒgƒiƒjƒkƒlƒm";
        strZenkaku += "ƒnƒqƒtƒwƒzƒ}ƒ~ƒ€ƒƒ‚ƒ„ƒ†ƒˆƒ‰ƒŠƒ‹ƒŒƒƒƒ’ƒ“ƒ@ƒBƒDƒFƒHƒƒƒ…ƒ‡ƒbAB[uv";

        // ‘÷“_A”¼‘÷“_¯•Ê—pƒf[ƒ^
        strHankakuDakuten = "Ş";
        strHankakuHanDakuten = "ß";
        ...
        for( i = 0 ; i < input.length() ; i++ )
        {
            ...
            if ( nIndex >= 0 )
            {
                ...
                // ‘÷‰¹•¶šA”¼‘÷‰¹•¶š‚Å‚È‚¯‚ê‚Î•’Ê‚ÌƒJƒ^ƒJƒi‚É•ÏŠ·
                if ( changedFlag != true )
                {
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ (”¼Šp”š¨‘SŠp”š) (”¼ŠpƒAƒ‹ƒtƒ@ƒxƒbƒg¨‘SŠpƒAƒ‹ƒtƒ@ƒxƒbƒg)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ (¬•¶šƒAƒ‹ƒtƒ@ƒxƒbƒg¨‘å•¶šƒAƒ‹ƒtƒ@ƒxƒbƒg)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ (‘å•¶šƒAƒ‹ƒtƒ@ƒxƒbƒg¨¬•¶šƒAƒ‹ƒtƒ@ƒxƒbƒg)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ i•¶š—ñ¨ŠG•¶šj
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @param carrierFlag ƒLƒƒƒŠƒAƒtƒ‰ƒO
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    static public String replaceEm(String input, int carrierFlag)
    {
        ...
        else if ( carrierFlag == DataMasterUseragent.CARRIER_AU )
        {
            ...
            input = replace( input, "<%=em108%>", "[iÓ°ÄŞ]" );
            input = replace( input, "<%=em109%>", "[iÓ°ÄŞ]" );
            input = replace( input, "<%=em110%>", "<img localsrc='108'>" );
            input = replace( input, "<%=em111%>", "[ÄŞºÓ]" );
            input = replace( input, "<%=em112%>", "[ÄŞºÓÎß²İÄ]" );
            ...
            input = replace( input, "<%=em168%>", "‚Ó‚­‚ë" );
            input = replace( input, "<%=em169%>", "<img localsrc='508'>" );
            input = replace( input, "<%=em170%>", "lŒ`" );
            input = replace( input, "<%=em171%>", "‚¢‚·" );
            ...
            input = replace( input, "<%=em177%>", "i±ÌßØ" );
            input = replace( input, "<%=em178%>", "i±ÌßØ" );
        ...
        else if ( carrierFlag == DataMasterUseragent.CARRIER_SOFTBANK )
        {
            ...
            input = replace( input, "<%=em29%>", "[Îß¹¯ÄÍŞÙ]" );
            ...
            input = replace( input, "<%=em93%>", "[Ò¶ŞÈ]" );
            input = replace( input, "<%=em94%>", "$F*" );
            input = replace( input, "<%=em95%>", "œ" );
            ...
            input = replace( input, "<%=em121%>", "[ˆÊ’uî•ñ]" );
            ...
            input = replace( input, "<%=em182%>", "[¼Ş°İ½Ş]" );
            input = replace( input, "<%=em183%>", "[½ÉÎŞ]" );
            ...
            input = replace( input, "<%=em193%>", "[»Œv]" );
            input = replace( input, "<%=em194%>", "$EV" );
            input = replace( input, "<%=em195%>", "$OX" );
            input = replace( input, "<%=em196%>", "[˜rŒv]" );
            ...
            input = replace( input, "<%=em243%>", "[¶ÀÂÑØ]" );
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ (APIŒü‚¯‚É‰üsƒR[ƒh‚ğ•ÏX)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    {
        // HTMLƒGƒXƒP[ƒv‚³‚ê‚½‚à‚Ì‚ğ–ß‚·
        input = DBEscape( input );
        // ‚»‚ê‚¼‚ê‚ğ‰üsƒR[ƒh‚É•ÏX
        input = input.replace( "\r\n", "" );
    ...
    /**
     * •¶š—ñ‚Ì’uŠ·‚ğs‚¤ (ƒnƒsƒzƒeƒ^ƒbƒ`Œü‚¯‚ÉƒGƒXƒP[ƒv•¶š‚ğ‘SŠp‚É)
     * 
     * @param input ˆ—‚Ì‘ÎÛ‚Ì•¶š—ñ
     * @return ’uŠ·ˆ—Œã‚Ì•¶š—ñ
     */
    ...
    /***
     * ƒJƒ^ƒJƒi‚ğ‚Ğ‚ç‚ª‚È‚É•ÏŠ·‚·‚é
     * 
     * @param input ƒJƒ^ƒJƒi‚Ì•¶š—ñ
     * @return
     */
    ...
    /**
     * IPƒAƒhƒŒƒX•ÏŠ· IPV4‚ÌƒAƒhƒŒƒXizz9.zz9.zz9.zz9j‚ğ16i•\‹L(FFFFFFFF)‚É•ÏŠ·
     * 
     * @param ip_address
     * @return ˆ—Œ‹‰Ê(FFFFFFFF)
     */
    public static String replaceHexIp(String ipAddress)
    {
        ...
        hexIp = hexIp.toUpperCase(); // ‘å•¶š‚É•ÏŠ·
        Logging.info( hexIp );
        return(hexIp);
    }
    ...
    /**
     * ƒ[ƒ‹ƒAƒhƒŒƒX‚Ìƒ}ƒXƒLƒ“ƒOˆ—iXXXXXXXX@XXXXXXXXXj‚ğXXX****@*******XXX‚É•ÏŠ·
     * 
     * @param mailAddress
     * @return ˆ—Œ‹‰Ê(String)
     */
