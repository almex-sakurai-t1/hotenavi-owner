...
import jp.happyhotel.owner.FormOwnerRsvOptionList;
import jp.happyhotel.owner.LogicOwnerRsvOptionList;
...
public class ActionOwnerRsvOptionList extends BaseAction
{
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        FormOwnerRsvOptionList dsp0; // form
        LogicOwnerRsvOptionList lgLPC; // logic

        dsp0 = new FormOwnerRsvOptionList();
        lgLPC = new LogicOwnerRsvOptionList();
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        ActionOwnerRsvArrivalOptionList action = new ActionOwnerRsvArrivalOptionList();
            ...
            request.setAttribute( "FORM_ReserveOptionListPC", dsp0 );
            strCarrierUrl = "owner_rsv_option_list.jsp";
            ...
            if ( dsp0.getRecCnt() > ActionOwnerRsvOptionList.listmax )
            {
                queryString = "actionReserveOptionListPC.act?selHotelIDValue=" + dsp0.getSelHotelID() +
                ...
                pageLinks = PagingDetails.getPagenationLink( dsp0.getPageAct(), ActionOwnerRsvOptionList.listmax, dsp0.getPageMax(), queryString );
                dsp0.setPageLink( pageLinks );
            }

            assignLinks = "actionReserveAssignListPC.act?selHotelIDValue=" + dsp0.getSelHotelID() +
                    "&dspFlg=" + dspFlg + "&date_f=" + dsp0.getDateFrom() +
                    "&date_t=" + setToDateVal + "&time_f_h=" + dsp0.getTimeFromHour() + "&time_f_m=" + dsp0.getTimeFromMin() + "&time_t_h=" + dsp0.getTimeToHour() + "&time_t_m=" + dsp0.getTimeToMin() + "&rsv_no=" + dsp0.getReserveNo();
            dsp0.setAssignLink( assignLinks );
     ...
     * @param frm
     */
    private void getDspHeader(HttpServletRequest request, FormOwnerRsvOptionList frm)
    {
        // 画面の値を取得
