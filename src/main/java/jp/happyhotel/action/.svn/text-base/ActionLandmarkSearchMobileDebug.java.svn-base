...
import jp.happyhotel.others.MapSpot1;
import jp.happyhotel.others.MasterSpot1;
...
public class ActionLandmarkSearchMobileDebug extends BaseAction
{
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        if ( (paramLocalId == null) || (paramLocalId.compareTo( "" ) == 0) || (CheckString.numCheck( paramLocalId ) == false) )
        {
            paramLocalId = "0";
        }
        ...
        if ( Integer.parseInt( paramSeq ) > 0 )
        {
            ...
            if ( (paramAcRead == null) && (carrierFlag == DataMasterUseragent.CARRIER_AU) )
            {
                try
                {
                    ...
                    ret = auCheck.authCheckForClass( request, "tokushu/transfer_premium.jsp?" + paramUidLink );
                ...
                {
                    Logging.info( "[ActionLandMarkSearchMobileDebug AuAuthCheck] Exception:" + e.toString() );
                }
            }
            ...
            {
                Logging.error( "[ActionLandMarkSearchMobileDebug dataLoginInfo] Exception:" + e.toString() );
            }
        }
        ...
        if ( Integer.parseInt( paramSpotId ) == 0 )
        {
            try
            {
                masterSpot = new MasterSpot1();
                mapSpot = new MapSpot1();
                queryString = "searchLandmarkMobileDebug.act?spot_id=0";
                ...
                if ( spotRet != false )
                {
                    ...
                    currentPageRecords = PagingDetails.getPageRecordsMobile( pageNum, SPOT_COUNT, masterSpot.getAllCount(), masterSpot.getCount(), dispFormat );
                    request.setAttribute( "PAGE_RECORDS", currentPageRecords );
                    if ( masterSpot.getAllCount() > SPOT_COUNT )
                    {
                        pageLinks = PagingDetails.getPagenationLinkMobile( pageNum, SPOT_COUNT, masterSpot.getAllCount(), queryString, paramUidLink );
            ...
            {
                Logging.error( "[ActionLandMarkSearchMobileDebug sendRedirect:error.jsp] Exception:" + e.toString() );
            }
        }
        // 検索ページ
        if ( Integer.parseInt( paramSeq ) == 0 )
        {
            mapSpot = new MapSpot1();
            queryString = "searchLandmarkMobileDebug.act?spot_id=" + paramSpotId + "&local_id=" + paramLocalId + "&pref_id=" + paramPrefId;
            // ランドマーク選択ページへ
            if ( Integer.parseInt( paramPrefId ) > 0 )
            {
                spotRet = mapSpot.getMapSpot( Integer.parseInt( paramSpotId ), Integer.parseInt( paramPrefId ), NOT_RECOMMEND, pageRecords, pageNum );
            ...
            try
            {
                ...
                if ( spotRet != false )
                {
                    ...
                    if ( Integer.parseInt( paramPrefId ) > 0 )
                    {
                        currentPageRecords = PagingDetails.getPageRecordsMobile( pageNum, pageRecords, mapSpot.getAllCount(), mapSpot.getCount(), dispFormat );
                        request.setAttribute( "PAGE_RECORDS", currentPageRecords );
                        if ( mapSpot.getAllCount() > pageRecords )
                        {

                            pageLinks = PagingDetails.getPagenationLinkMobile( pageNum, pageRecords, mapSpot.getAllCount(), queryString, paramUidLink );
                            request.setAttribute( "PAGE_LINK", pageLinks );
                        }
                    }
                }
            ...
            {
                Logging.error( "[ActionLandMarkSearchMobileDebug sendRedirect:" + sendUrl + "] Exception:" + e.toString() );
            }
        }
        // 検索結果
        else if ( Integer.parseInt( paramSeq ) > 0 )
        {
            ...
            queryString = "searchLandmarkMobileDebug.act?spot_id=" + paramSpotId + "&seq=" + paramSeq + "&scale=" + paramScale + "&empty=" + paramEmpty;
            ...
            if ( spotRet != false && mapSpot.getAllCount() > 0 )
            {
                ...
                if ( Integer.parseInt( paramId ) == 0 )
                {
                    ...
                    // {
                    cu.setCircle( "R" + (ConvertScale.getScaleFromLevel( Integer.parseInt( paramScale ) ) * 120) / 10000 + ":BFFFFFF:W1:PWGS84," + strLat + "," + strLon );
                    // }
                ...
                if ( ret != false )
                {
                    try
                    {
                        ...
                        cg.convertDms2Degree( this.lat, this.lon );
                        ...
                        if ( Integer.parseInt( paramId ) > 0 )
                        {
                            ...
                            ret = she.getSearchHotel( coordinateDlLat, coordinateDlLon, coordinateUrLat, coordinateUrLon, Double.parseDouble( strLat ), Double.parseDouble( strLon ),
                        ...
                        {
                            // 今回はDBで取得したLat、Lonを使用していく（yahooAPIで返ってくるLat、Lonが微妙にずれているため）
                            ret = she.getSearchHotel( coordinateDlLat, coordinateDlLon, coordinateUrLat, coordinateUrLon, Double.parseDouble( strLat ), Double.parseDouble( strLon ), Integer.parseInt( paramEmpty ), ConvertScale.getScaleFromLevel( Integer
                                    .parseInt( paramScale ) ) );
                        ...
                        if ( ret != false )
                        {
                            ...
                            if ( dhd != null )
                            {
                                ...
                                for( i = dhd.length - 1 ; i >= 0 ; i-- )
                                {
                                    ...
                                    {
                                        icon += "II" + (3 * i + 14) + "G:PWGS84," + dhd[i].getHotelLat() + "," + dhd[i].getHotelLon();
                                        strPosIcon = "II" + (3 * i + 14) + "G:PWGS84," + dhd[i].getHotelLat() + "," + dhd[i].getHotelLon();
                                    }
                                    // 空室
                                    else if ( arrDataSearchHotel[i].getEmptyStatus() == 1 )
                                    {
                                        icon += "II" + (3 * i + 12) + "G:PWGS84," + dhd[i].getHotelLat() + "," + dhd[i].getHotelLon();
                                        strPosIcon = "II" + (3 * i + 12) + "G:PWGS84," + dhd[i].getHotelLat() + "," + dhd[i].getHotelLon();
                                    }
                                    // 満室
                                ...
                                {
                                    cu.setScale( ConvertScale.getScaleFromLevel( Integer.parseInt( paramScale ) ) );
                                    cu.setC( "WGS84," + paramCenter );
                                    // if ( paramCircle.compareTo( "true" ) == 0 )
                                    // {
                                    // 拡大前の縮尺で半径を表示する
                                    cu.setCircle( "R" + (Integer.parseInt( this.um.getUserMapInfo().getScale() ) * 120) / 10000 + ":BFFFFFF:W1:PWGS84," + strLat + "," + strLon );
                                    // }

                                    strDispMapUrl = cu.getDrawMap();
                                    ret = con.urlConnection( request, response, strDispMapUrl );

                                    queryString = "searchLandmarkMobileDebug.act?spot_id=" + paramSpotId + "&seq=" + paramSeq +
                                            "&scale=" + Integer.toString( ConvertScale.getLevelFromScale( Integer.parseInt( this.um.getUserMapInfo().getScale() ) ) ) + "&empty=" + paramEmpty;
                    ...
                    {
                        Logging.error( "[ActionLandMarkSearchMobileDebug result] Exception:" + e.toString() );
                    }
                }
                ...
                {
                    Logging.error( "[ActionLandMarkSearchMobileDebug requestDispatcher:" + sendUrl + "] Exception:" + e.toString() );
                }
            }
            else
            {
    ...
    private void replaceGeodesic(String coordinate)
    {
        ...
        if ( nIndex != -1 )
        {
            this.lat = coordinate.substring( 0, nIndex );
            this.lat = this.lat.replaceAll( "/", "." );
            this.lon = coordinate.substring( nIndex + 1 );
