...
public class ActionApiReserveSearch extends BaseAction
{
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            ...
            // 対象地域の予約対応ﾎﾃﾙ取得
            hotelIdList = searchArea.getSearchHappieIdListByPrefId( Integer.parseInt( paramPrefId ), RSV_HAPPIE_MEMBER_HOTEL );

            if ( hotelIdList != null && hotelIdList.length > 0 )
            {
                searchHotelDao = new SearchHotelDao_M2();
                // 予約ハピー加盟店用のホテルリストを取得する
                searchHotelDao.getRsvHappieList( hotelIdList, pageRecords, Integer.parseInt( paramPage ) );
                // ﾎﾃﾙ情報取得
        ...
        {
            Logging.error( "[ActionApiReserveSearch.execute() ] Exception:", exception );
            ...
            {
                Logging.error( "[ActionApiReserveSearch response]Exception:" + e.toString() );
            }
        }
        finally
        {
