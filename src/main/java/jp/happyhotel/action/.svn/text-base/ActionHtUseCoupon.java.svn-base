...
import jp.happyhotel.others.GenerateXmlHapiTouchHtUseCoupon;
import jp.happyhotel.touch.TouchCi;
import jp.happyhotel.touch.TouchUserCoupon;

/**
 * ハピホテアプリ（ハピホテタッチクーポン使用）
 * 
 * @author S.Tashiro
 * @version 1.0 2014/08/26
 * 
 */
public class ActionHtUseCoupon extends BaseAction
{
    final int                   USED               = 1;
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        int errorCode = 0;
        GenerateXmlHapiTouchHtUseCoupon gxTouch = null;
        ...
        try
        {
            ...
            if ( Integer.parseInt( paramSeq ) > 0 && Integer.parseInt( paramId ) > 0 && Integer.parseInt( paramCouponSeq ) > 0 )
            {
                ...
                if ( ret != false )
                {
                    ...
                    // {
                    // boolCoupon = tuc.updateDeclareCoupon( Integer.parseInt( paramId ), Integer.parseInt( paramCouponNo ), Integer.parseInt( paramCouponSeq ), Integer.parseInt( paramUsed ) );
                    ...
                    if ( boolCoupon != false )
                    {
                        ...
                        dhc = this.getHotelCoupon( Integer.parseInt( paramId ), duc.getSeq() );
                        ...
                        datc.setUserSeq( duc.getSeq() );

                        // マスタークーポンから取得したデータがあれば
                        if ( dhc != null )
                        {
                            // ホテルクーポンのデータを取得
                            datc.setHotelSeq( dhc.getSeq() );
                        }

                        if ( ret != false )
                        {
                            datc.updateData( Integer.parseInt( paramId ), Integer.parseInt( paramCouponSeq ) );
                        }
                        else
                        {
                            datc.insertData();
            ...
            gxTouch = new GenerateXmlHapiTouchHtUseCoupon();
            if ( boolCoupon != false )
            {
