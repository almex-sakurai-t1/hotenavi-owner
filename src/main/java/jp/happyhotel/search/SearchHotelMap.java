/*
 * @(#)SearchHotelMap.java 1.00 2007/09/21 Copyright (C) ALMEX Inc. 2007 地図検索ホテル取得クラス
 */
 ...
 */
public class SearchHotelMap implements Serializable
{
    private static final long serialVersionUID   = -1520912486828994815L;

    private static final int  LATLON_RANGE       = 100000;
    private static final int  LATLON_RANGE_SMALL = 30000;
     ...
     * ホテル一覧情報取得(ホテルランク順)
     * 
     * @param lat 緯度(世界測地系 999.999999)
     * @param lon 経度(世界測地系 99.999999)
    ...
    public boolean getHotelList(String lat, String lon, int countNum, int pageNum)
    {
        ...
        subQuery = " hh_hotel_basic.id, hh_hotel_basic.rank, hh_hotel_basic.kind," +
                ...
                " hh_hotel_basic.ad_pref_id, hh_hotel_basic.renewal_date_text, hh_hotel_basic.new_open_search_flag, (CASE WHEN hh_hotel_basic.rank=3 THEN 2 ELSE hh_hotel_basic.rank END) AS Ranking";
        ...
        query = query + " ,hh_hotel_sort WHERE";
        ...
        {
            Logging.error( "[SearchHotelMap.getHotelList] Exception=" + e.toString() );
            return(false);
        }
        finally
        {
        ...
        // ホテル総件数の取得
        query = "SELECT hh_hotel_basic.id FROM hh_hotel_basic WHERE";
        query = query + " (hotel_lat_num > ? AND hotel_lat_num < ?)";
        query = query + " AND ";
        query = query + " (hotel_lon_num > ? AND hotel_lon_num < ?)";
        query = query + " AND hh_hotel_basic.kind <= 7";
     ...
     * ホテル一覧情報取得(ホテルランク順)
     * 
     * @param leftLat 緯度(世界測地系 999.999999)
     * @param leftLon 経度(世界測地系 99.999999)
     * @param rightLat 緯度(世界測地系 999.999999)
     * @param rightLon 経度(世界測地系 99.999999)
     * @param countNum 取得件数（0：全件 ※pageNum無視）
     * @param pageNum ページ番号（0～）
     * @return 処理結果(TRUE:正常,FALSE:異常)
     */
    public boolean getHotelListByRange(String leftLat, String leftLon, String rightLat, String rightLon, int countNum, int pageNum)
    {
        int i;
        int count;
        int latLeft;
        ...
        // 左端
        latD = Double.valueOf( leftLat );
        lonD = Double.valueOf( leftLon );
        ...
        latLeft = cg.getLatTOKYONum() - LATLON_RANGE_SMALL;
        lonLeft = cg.getLonTOKYONum() - LATLON_RANGE_SMALL;

        cg = new ConvertGeodesic();

        // 右端
        latD = Double.valueOf( rightLat );
        lonD = Double.valueOf( rightLon );
        ...
        latRight = cg.getLatTOKYONum() + LATLON_RANGE_SMALL;
        lonRight = cg.getLonTOKYONum() + LATLON_RANGE_SMALL;
