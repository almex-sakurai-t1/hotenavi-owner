 ...
 */
public class SearchHotelCity implements Serializable
{
     ...
     */
    public boolean getHotelList(int jisCode, int countNum, int pageNum)
    {
        int i;
        int count;
        String query;
        ...
        query = query + " ,hh_hotel_pv WHERE jis_code = ?";
        ...
        // ホテル総件数の取得
        query = "SELECT id FROM hh_hotel_basic WHERE jis_code = ?";
        query = query + " AND hh_hotel_basic.kind <= 7";
        count = 0;
        try
        {
        ...
        // 市区町村の取得
        this.m_cityInfo = new DataMasterCity();
        this.m_cityInfo.getData( jisCode );
        return(true);
    }
