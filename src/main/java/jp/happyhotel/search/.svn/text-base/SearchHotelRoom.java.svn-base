/*
 * @(#)SearchHotelRoom.java 1.00 2007/09/21 Copyright (C) ALMEX Inc. 2007 部屋画像検索クラス
 */
 ...
 */
public class SearchHotelRoom implements Serializable
{
    private static final long serialVersionUID = 590842811180729164L;
    ...
    public boolean getHotelList(int[] hotelIdList, int countNum, int pageNum)
    {
        ...
        query = query + " , hh_hotel_sort, hh_hotel_room";
        query = query + " WHERE hh_hotel_basic.kind <= 7";
        query = query + " AND hh_hotel_basic.rank > 1";
        query = query + " AND hh_hotel_basic.id = hh_hotel_sort.id";
        query = query + " AND hh_hotel_basic.id = hh_hotel_room.id";
        ...
        query = query + " AND hh_hotel_room.disp_flag = 1";
        ...
        // ホテル総件数の取得
        query = "SELECT hh_hotel_basic.id, (CASE WHEN hh_hotel_basic.rank=3 THEN 2 ELSE hh_hotel_basic.rank END) AS Ranking FROM hh_hotel_basic, hh_hotel_sort, hh_hotel_room ";
        query = query + " WHERE hh_hotel_basic.kind <= 7";
        query = query + " AND hh_hotel_basic.rank > 1";
        query = query + " AND hh_hotel_basic.id = hh_hotel_sort.id";
        query = query + " AND hh_hotel_basic.id = hh_hotel_room.id";
