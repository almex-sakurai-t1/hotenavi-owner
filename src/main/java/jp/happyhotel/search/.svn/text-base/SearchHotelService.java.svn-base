/*
 * @(#)SearchHotelService.java 1.00 2007/07/18 Copyright (C) ALMEX Inc. 2007 ホテルサービス別ホテル取得クラス
 */
 ...
 */
public class SearchHotelService implements Serializable
{
    private static final long serialVersionUID = 6224112837394092536L;
     ...
     */
    public boolean getHotelList(int[] service, int countNum, int pageNum)
    {
        int i;
        int count;
        String query;
        ...
        query = query + ",hh_hotel_pv WHERE hh_hotel_basic.id <> 0";

        if ( service[0] == 1 )
        {
            query = query + " AND (";
            query = query + " hh_hotel_basic.credit = 1";
            query = query + " OR hh_hotel_basic.credit_visa = 1";
            query = query + " OR hh_hotel_basic.credit_master = 1";
            query = query + " OR hh_hotel_basic.credit_jcb = 1";
            query = query + " OR hh_hotel_basic.credit_dc = 1";
            query = query + " OR hh_hotel_basic.credit_nicos = 1";
            query = query + " OR hh_hotel_basic.credit_amex = 1";
            query = query + " OR hh_hotel_basic.credit_etc <> ''";
            query = query + " )";
        }
        if ( service[1] == 1 )
        {
        ...
        if ( service[2] == 1 )
        {
            query = query + " AND hh_hotel_basic.coupon = 1";
        }
        if ( service[3] == 1 )
        {
        ...
        {
            query = query + " AND (";
            query = query + " hh_hotel_basic.reserve = 1";
            query = query + " OR hh_hotel_basic.reserve_tel = 1";
            query = query + " OR hh_hotel_basic.reserve_mail = 1";
            query = query + " OR hh_hotel_basic.reserve_web = 1";
            query = query + " )";
        }
        if ( service[5] == 1 )
        {
        ...
        query = query + " LEFT JOIN hh_hotel_status";
        query = query + " ON hh_hotel_basic.id = hh_hotel_status.id";
        query = query + " WHERE hh_hotel_basic.id <> 0";
