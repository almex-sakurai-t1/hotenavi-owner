/*
 * @(#)SearchHotelPrice.java 1.00 2007/07/18 Copyright (C) ALMEX Inc. 2007 料金検索ホテル取得クラス
 */
 ...
 */
public class SearchHotelPrice implements Serializable
{
     ...
     */
    public boolean getHotelList(int[] hotelIdList, int[] priceKind, int startPrice, int endPrice, int countNum, int pageNum)
    {
        int i;
        int count;
        boolean ret;
        ...
        query = "SELECT " + subQuery + " FROM hh_hotel_price,hh_hotel_basic";
        query = query + " LEFT JOIN hh_hotel_status";
        query = query + " ON hh_hotel_basic.id = hh_hotel_status.id";
        query = query + " ,hh_hotel_pv WHERE";
        query = query + " hh_hotel_price.price_from >= " + startPrice;
        query = query + " AND hh_hotel_price.price_to <= " + endPrice;
        query = query + " AND hh_hotel_price.start_date <= " + DateEdit.getDate( 2 );
        query = query + " AND hh_hotel_price.end_date >= " + DateEdit.getDate( 2 );
        query = query + " AND hh_hotel_price.data_flag = 1";
        ...
        // ホテル総件数の取得
        query = "SELECT hh_hotel_basic.id FROM hh_hotel_price,hh_hotel_basic,hh_hotel_pv WHERE";
        query = query + " hh_hotel_price.price_from >= " + startPrice;
        query = query + " AND hh_hotel_price.price_to <= " + endPrice;
        query = query + " AND hh_hotel_price.start_date <= " + DateEdit.getDate( 2 );
        query = query + " AND hh_hotel_price.end_date >= " + DateEdit.getDate( 2 );
     ...
     * @return 処理結果(TRUE:正常,FALSE:異常)
     */
    public boolean getHotelListByTime(int[] hotelIdList, int[] priceKind, int startTime, int endTime, int countNum, int pageNum)
    {
        int i;
        int count;
        boolean ret;
        ...
        if ( startTime == 0 && endTime == 2400 )
        {
            ...
            query = query + " AND hh_hotel_price.time_to = " + endTime;
        }
        else
        {
            query = query + " hh_hotel_price.time_from >= " + startTime;
        ...
        // ホテル総件数の取得
        query = "SELECT hh_hotel_basic.id FROM hh_hotel_price,hh_hotel_basic WHERE";
        if ( startTime == 0 && endTime == 2400 )
        {
            query = query + " hh_hotel_price.time_from = " + startTime;
            query = query + " AND hh_hotel_price.time_to = " + endTime;
