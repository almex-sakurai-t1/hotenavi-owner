 ...
 */
public class SearchHotelChain implements Serializable
{
     ...
     * チェーン店検索結果取得(ホテルランク順)
     * 
     * @param groupId グループID(0:全件検索)
     * @param countNum 取得件数（0：全件 ※pageNum無視）
     * @param pageNum ページ番号（0～）
     * @return 処理結果(TRUE:正常,FALSE:異常)
     */
    public boolean getHotelList(int groupId, int countNum, int pageNum)
    {
        int i;
        int count;
        boolean ret;
        ...
        query = "SELECT " + subQuery + " FROM hh_hotel_chain,hh_hotel_basic";
        query = query + " LEFT JOIN hh_hotel_status";
        query = query + " ON hh_hotel_basic.id = hh_hotel_status.id";
        query = query + " WHERE hh_hotel_chain.group_id = ?";
        query = query + " AND hh_hotel_basic.id = hh_hotel_chain.id";
        query = query + " AND hh_hotel_basic.kind <= 7";
        query = query + " AND hh_hotel_chain.start_date <= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        query = query + " AND hh_hotel_chain.end_date >= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        query = query + " GROUP BY hh_hotel_basic.id";
        query = query + " ORDER BY hh_hotel_chain.disp_index, hh_hotel_basic.rank DESC, hh_hotel_basic.name_kana";
        ...
        {
            Logging.info( "[SearchHotelChain.getHotelList] Exception=" + e.toString() );
            ret = false;
        }
        finally
        {
        ...
        query = "SELECT hh_hotel_basic.id FROM hh_hotel_chain,hh_hotel_basic WHERE";
        query = query + " hh_hotel_chain.group_id = ?";
        query = query + " AND hh_hotel_basic.id = hh_hotel_chain.id";
        query = query + " AND hh_hotel_basic.kind <= 7";
        query = query + " AND hh_hotel_chain.start_date <= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        query = query + " AND hh_hotel_chain.end_date >= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        ...
        {
            Logging.error( "[SearchHotelChain.getHotelList] Exception=" + e.toString() );
            ret = false;
        }
        finally
        {
     ...
     * 
     * @param groupId グループID(0:全件検索)
     * @param prefId 都道府県コード(0:全件検索)
     * @param countNum 取得件数（0：全件 ※pageNum無視）
     * @param pageNum ページ番号（0～）
     * @return 処理結果(TRUE:正常,FALSE:異常)
     */
    public boolean getHotelListByPref(int groupId, int prefId, int countNum, int pageNum)
    {
        int i;
        int count;
        boolean ret;
        ...
        query = query + " ON hh_hotel_basic.id = hh_hotel_status.id";
        query = query + " WHERE hh_hotel_chain.group_id = ?";
        query = query + " AND hh_hotel_basic.ad_pref_id = ?";
        query = query + " AND hh_hotel_basic.id = hh_hotel_chain.id";
        query = query + " AND hh_hotel_basic.kind <= 7";
        query = query + " AND hh_hotel_chain.start_date <= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        query = query + " AND hh_hotel_chain.end_date >= " + Integer.parseInt( DateEdit.getDate( 2 ) );
    ...
    public boolean getChainList()
    {
        ...
        query = "SELECT hh_master_chain.group_id,hh_master_chain.name,hh_master_chain.name_kana,count(*) as cnt FROM hh_master_chain,hh_hotel_chain WHERE";
        query = query + " hh_hotel_chain.group_id=hh_master_chain.group_id";
        query = query + " AND hh_master_chain.disp_flag = 0";
        query = query + " AND hh_hotel_chain.start_date <= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        query = query + " AND hh_hotel_chain.end_date >= " + Integer.parseInt( DateEdit.getDate( 2 ) );
        query = query + " GROUP BY hh_master_chain.group_id";
        query = query + " ORDER BY cnt DESC";
        ...
        try
        {
            ...
            if ( result != null )
            {
                ...
                while( result.next() != false )
                {
                    this.m_groupIdList[count] = result.getInt( "hh_master_chain.group_id" );
                    this.m_groupNameList[count] = result.getString( "hh_master_chain.name" );
                    this.m_groupNameKanaList[count] = result.getString( "hh_master_chain.name_kana" );
                    this.m_groupIdCount[count] = result.getInt( "cnt" );
        ...
        {
            Logging.info( "[SearchHotelChain.getChainList] Exception=" + e.toString() );
            ret = false;
        }
        finally
        {
