...
<%
    ...
    for( i = 0 ; i < count ; i++ )
    {
        ...
        // 車番（地域）
        if( ownerinfo.CarArea[i].compareTo("") != 0 )
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<B>" + ownerinfo.CarArea[i] + "</B>&nbsp;";
        }
        else
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "&nbsp;";
        }

        // 車番（車種）
        if( ownerinfo.CarType[i].compareTo("000") != 0 )
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<B>" + ownerinfo.CarType[i] + "</B><BR>";
        }
        else
        {
        ...
        // 車番（種別）
        if( ownerinfo.CarKind[i].compareTo("") != 0 )
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<B>" + ownerinfo.CarKind[i] + "</B>&nbsp;";
        }
        else
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "&nbsp;";
        ...
        // 車番
        if( ownerinfo.CarNo[i].compareTo("0000") != 0 )
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<B>" + ownerinfo.CarNo[i] + "</B><BR>";
        }
        else
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<BR>";
        ...
        // 駐車場番号
        if( ownerinfo.CarParkingNo[i] != 0 )
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<B>" + ownerinfo.CarParkingNo[i] + "</B><BR>";
        }
        else
        {
            roomtag[x][y][z] = roomtag[x][y][z] + "<BR>";
