    ...
    <table>
      ...
      <tr>
        <td class="size10" align="center">
          <a <%if (beforYearLink) {%>class="blue" href="#" onclick="jumpCompareDate(<%=beforYearDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>><<前年</a>&nbsp;
          <a <%if (beforMonthLink){%>class="blue" href="#" onclick="jumpCompareDate(<%=beforMonthDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>><前月</a>&nbsp;&nbsp;
          <a <%if (nextMonthLink) {%>class="blue" href="#" onclick="jumpCompareDate(<%=nextMonthDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>>翌月></a>&nbsp;
          <a <%if (nextYearLink)  {%>class="blue" href="#" onclick="jumpCompareDate(<%=nextYearDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>>翌年>></a>
        </td>
      </tr>
      <tr>
        <td class="cal_title">
...
%>
      ...
      <tr>
        <td class="size10" colspan="2"  align="center">
          <a <%if (beforWeekLink) {%>class="blue" href="#" onclick="jumpCompareDate(<%=beforWeekDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>><<前週</a>&nbsp;
          <a <%if (beforDayLink) {%>class="blue" href="#" onclick="jumpCompareDate(<%=beforDayDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>><前日</a>&nbsp;&nbsp;
          <a <%if (nextDayLink) {%>class="blue" href="#" onclick="jumpCompareDate(<%=nextDayDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>>翌日></a>&nbsp;
          <a <%if (nextWeekLink) {%>class="blue" href="#" onclick="jumpCompareDate(<%=nextWeekDate%>,'<%=CompareSubmit%>');"<%}else{%>class="gray"<%}%>>翌週>></a>
        </td>
      </tr>
    </table>
  </td>
...
<form name="Compareselectday" action="salesdisp_select.jsp?Kind=<%=paramKind%>" method="post" target="mainFrame">
  <TD class="size10" align="left" nowrap  bgcolor="#A8BEBC" colspan=2>
    ...
    <input class="size12 ar" name="CompareStartDay"   id="CompareStartDay" maxlength="2" size="2" value="<%=start_day%>" onclick="this.select();" onchange="checkCompareDay(this.value);" onchange="checkCompareDay(this.value);">日
    <input name="Compare" type="hidden" value="<%=CompareMode%>">
    <INPUT type="button" class="size12" value="照会"  onclick="if(checkCompareDate()){submitCompareDate();}">
  </TD>
</tr>
