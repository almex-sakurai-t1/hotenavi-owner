...
<title>月次日別売上(<%= StringEscapeUtils.escapeHtml4(hotelid) %>)年月指定</title>
...
<h2>☆月次日別売上</h2>
<ul class="link">
（<%= addup_date / 10000 %>/<%= nf.format(addup_date / 100 % 100) %>）
<li><a href="<%= response.encodeURL("salesmonthlistdisp.jsp?HotelId=" + URLEncoder.encode(hotelid, "Windows-31J")) %>">本月</a></li>
</ul>
<%-- 日付選択アイテムを表示します --%>
