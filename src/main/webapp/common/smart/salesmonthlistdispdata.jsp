...
%>
<h2><%= (ownerinfo.SalesGetStartDate / 10000) %>/<%= (ownerinfo.SalesGetStartDate / 100 % 100) %><br />
...
<%
    for( i = 1 ; i <= last_day[start_month - 1] ; i++ )
    {
        ...
        {
           // 売上情報取得
           ownerinfo.sendPacket0102(1, hotelid, 1);
        }
...
%>
<tr>
<td><a href="<%= response.encodeURL("salesdisp.jsp?HotelId=" + URLEncoder.encode(hotelid, "Windows-31J") + "&Year=" + start_year + "&Month=" + start_month + "&Day=" + i) %>"><%= sf.rightFitFormat( Integer.toString(i), 2) %>(<%= arrWday[wday] %>)</a></td><td class="uriage"><%= sf.rightFitFormat( Kanma.get(ownerinfo.SalesTotal), 10 ) %></td><td class="uriage"><%= sf.rightFitFormat( Integer.toString(ownerinfo.SalesTotalCount), 5) %><font size="-1">組</font></td>
</tr>
<%
    }
%>
